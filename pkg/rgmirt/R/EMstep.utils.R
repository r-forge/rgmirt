...
Mstep <- function(pars, est, longpars, ngroups, J, gTheta, itemloc, PrepList, L, ANY.PRIOR,
                  ...
                  lrPars, keep_vcov_PD, control, nconstrain = NULL, lambda){
    p <- longpars[est]
    if(length(p)){
        if(Moptim == 'BFGS'){
            ...
            opt <- try(optim(p, fn=Mstep.LL, gr=Mstep.grad, method='BFGS', control=control,
                             ...
                             ANY.PRIOR=ANY.PRIOR, itemloc=itemloc, keep_vcov_PD=keep_vcov_PD, lambda = lambda),
                       silent=TRUE)
...
Mstep.LL <- function(p, est, longpars, pars, ngroups, J, gTheta, PrepList, L, CUSTOM.IND,
                     SLOW.IND, constrain, nconstrain, itemloc, DERIV, rlist, ANY.PRIOR, keep_vcov_PD, lambda=c(0,0)){
    longpars[est] <- p
    longpars <- longpars_constrain(longpars=longpars, constrain=constrain,
                                   nconstrain=nconstrain)
    ...
    beta = unlist(sapply(pars[[1]], slot, 'par'))
    
    
    t.beta <<- beta
    t.est <<- est
    
    l1 = lambda[1] * sum(abs(beta[est]))
    l2 = lambda[2] * sum(beta^2)
    
    return(-sum(LLs) + l1 + l2)
}
...
Mstep.grad <- function(p, est, longpars, pars, ngroups, J, gTheta, PrepList, L, ANY.PRIOR,
                       constrain, nconstrain, itemloc, DERIV, rlist, CUSTOM.IND, SLOW.IND, keep_vcov_PD, lambda){
    longpars[est] <- p
    longpars <- longpars_constrain(longpars=longpars, constrain=constrain,
                                   nconstrain=nconstrain)
    pars <- reloadPars(longpars=longpars, pars=pars, ngroups=ngroups, J=J)
    ...
    beta = unlist(sapply(pars[[1]], slot, 'par'))
    g.l1 = lambda[1] * sign(beta)
    g.l2 = 2 * lambda[2] * beta

    return(-grad[est]+g.l1[est]+g.l2[est])
}
